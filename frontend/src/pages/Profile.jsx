...
export default function Profile() {
  ...
  if (!data?.user) return <div className="p-10 text-center font-black">{t('profile.notFound')}</div>;
  const u = data.user;
  ...
  return (
    <div className="max-w-2xl mx-auto pb-6">
      <div className="flex items-center gap-2 px-3 py-2 sticky top-0 z-10 backdrop-blur-xl bg-white/85 dark:bg-black/85 md:static md:bg-transparent">
        ...
        <div className="font-black text-lg truncate flex-1">@{u.username}</div>
      ...
      <div className="px-4 pt-2">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 flex justify-around text-center">
            {[[posts.length, t('profile.posts')], [u.followers, t('profile.followers')], [u.following, t('profile.following')]].map(([n, l], i) => (
              <div key={i}><div className="font-black text-lg">{fmt.n(n)}</div><div className="text-xs opacity-60">{l}</div></div>
            ))}
          </div>
        </div>

        <div className="mt-3">
          <div className="font-black flex items-center gap-1.5">{u.name}{u.verified && <Verified />}</div>
        ...
        <div className="flex gap-2 mt-3">
          {data.isSelf ? (
            ...
            <>
              <button onClick={follow} className={`flex-1 font-bold rounded-2xl px-5 py-2.5 transition flex items-center justify-center gap-2 ${following ? 'bg-black/5 dark:bg-white/10' : 'zivv-gradient text-white shadow-pop'}`}>
        ...
        <div className="flex mt-4 border-b border-black/10 dark:border-white/10">
          {[['posts', <GridIcon key="g" size={19} />, t('profile.posts')], ['reels', <FilmIcon key="f" size={19} />, t('profile.reels')], ['store', <BagIcon key="b" size={19} />, t('profile.store')]].map(([v, icon, l]) => (
            <button key={v} onClick={() => setTab(v)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-black border-b-2 -mb-px transition ${tab === v ? 'border-current' : 'border-transparent opacity-40'}`}>
              {icon}{l}
            </button>
          ))}
        </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {(data.listings || []).map((l) => (
                  <Link key={l.id} to={`/market/${l.id}`} className="card overflow-hidden">
                    <div className="aspect-square zivv-gradient"><ZImg seed={`listing-${l.id}`} w={400} h={400} className="w-full h-full object-cover" alt="" /></div>
