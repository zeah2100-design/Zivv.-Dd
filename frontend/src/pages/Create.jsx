...
import { ImageIcon, FilmIcon, BagIcon, SendIcon, SparklesIcon } from '../components/icons';
...
export default function Create() {
  ...
  const tabs = [
    ['post', '✍️', t('create.tabPost')],
    ['story', '⭕', t('create.tabStory')],
    ['reel', '🎬', t('create.tabReel')],
    ['listing', '🏷️', t('create.tabListing')],
  ];

  return (
    <div className="p-3 md:p-4 max-w-2xl mx-auto space-y-3">
      <h1 className="font-black text-2xl px-1">{t('create.title')}</h1>
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
        {tabs.map(([v, e, l]) => (
          <button key={v} onClick={() => setKind(v)}
            className={`px-4 py-2 text-sm font-black rounded-full transition shrink-0 ${kind === v ? 'tab-active' : 'bg-black/5 dark:bg-white/10 opacity-60'}`}>
            {e} {l}
          </button>
        ))}
      </div>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <textarea value={text} onChange={(e) => setText(e.target.value)} rows={5} maxLength={2000}
                placeholder={t('create.captionPh')} className="w-full bg-transparent resize-none text-[16px] mt-1 placeholder:opacity-40 focus:outline-none" />
            </div>
          </div>
      ...
      {kind === 'reel' && (
        <div className="card p-8 text-center">
          ...
          <div className="font-black text-lg">{t('create.reelSoon')}</div>
      ...
      {kind === 'listing' && (
        <div className="card p-4 space-y-3">
          <div className="flex items-center gap-2 font-black"><BagIcon size={20} />{t('create.newListing')}</div>
      ...
      {!!done && <div className="card p-4 text-center font-black text-green-500 fade-in">{done}</div>}
    </div>
  );
}
