...
class SupabaseClient {
  ...
  async updateGoldReq(id, status) {
    return this.update("gold_reqs", `id=eq.${encodeURIComponent(id)}`, { status });
  }

  async updateAccountPassword(email, hash) {
    return this.update("accounts", `email=eq.${encodeURIComponent(String(email).toLowerCase())}`, { password_hash: hash });
  }

  async getReactions(msgId) {
    if (msgId) return this.query("msg_reactions", `msg_id=eq.${encodeURIComponent(msgId)}&order=created_at.asc&limit=1000`);
    return this.query("msg_reactions", `select=*&order=created_at.desc&limit=2000`);
  }

  async toggleReaction(msgId, userKey, emoji) {
    const id = `${msgId}:${String(userKey).toLowerCase()}:${emoji}`;
    const ex = await this.query("msg_reactions", `id=eq.${encodeURIComponent(id)}&limit=1`);
    if (ex && ex.length) { await this.delete("msg_reactions", `id=eq.${encodeURIComponent(id)}`); return { on: false }; }
    await this.upsert("msg_reactions", { id, msg_id: msgId, user_key: String(userKey).toLowerCase(), emoji }, "id");
    return { on: true };
  }

  async getMessageById(id) {
    const rows = await this.query("messages", `id=eq.${encodeURIComponent(id)}&limit=1`);
    return (rows && rows[0]) || null;
  }

  async deleteMessage(id) {
    await this.delete("msg_reactions", `msg_id=eq.${encodeURIComponent(id)}`);
    return this.delete("messages", `id=eq.${encodeURIComponent(id)}`);
  }

  async markMessagesRead(threadUser, peer) {
    return this.update("messages", `thread_user=eq.${encodeURIComponent(threadUser)}&from_user=eq.${encodeURIComponent(peer)}`, { read: true });
  }

  async updateFriendReq(id, status) {
    return this.update("friend_reqs", `id=eq.${encodeURIComponent(id)}`, { status });
  }

  async getPrivAuth(user) {
    const rows = await this.query("priv_auth", `user_key=eq.${encodeURIComponent(String(user).toLowerCase())}&limit=1`);
    return (rows && rows[0]) || null;
  }

  async setPrivAuth(user, patch) {
    return this.upsert("priv_auth", { user_key: String(user).toLowerCase(), ...(patch || {}) }, "user_key");
  }

  async getModActions() { return this.query("mod_actions", `select=*&order=created_at.desc&limit=500`); }
  async createModAction(a) { return this.upsert("mod_actions", a, "id"); }
  async getAuditLog() { return this.query("audit_log", `select=*&order=created_at.desc&limit=500`); }
  async createAuditLog(a) { return this.upsert("audit_log", a, "id"); }
