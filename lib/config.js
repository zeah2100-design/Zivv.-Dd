...
function getConfig() {
  ...
  return {
    ...
    ai: {
      // المفتاح لازم ييجي من متغيرات البيئة فقط — env only
      // Vercel: Settings > Environment Variables > GEMINI_API_KEY = <your key>
      // محلي: انسخ .env.example إلى .env وحط المفتاح فيه
      cometKey:
        process.env.GEMINI_API_KEY ||
        process.env.COMET_API_KEY ||
        process.env.AI_API_KEY ||
        "",
      // DEPRECATED_FALLBACK — احذف السطر ده بعد ما تضيف GEMINI_API_KEY على Vercel
      cometKeyLegacy: process.env.ZIVV_LEGACY_AI_KEY || "",
      cometModel:
