...
class Database {
  ...
  async getPostById(id) { return this.client.getPostById(id); }
  async createPost(post) { return this.client.createPost(post); }
  async deletePost(id) { return this.client.deletePost(id); }

  // Views
  async recordView(postId, userKey) { return this.client.recordView(postId, userKey); }
  async getViews(postId) { return this.client.getViews(postId); }

  // Stats
  async getStats() { return this.client.getStats(); }

  // Seed + ephemeral (serverless)
  async ensureSeeded() {
    if (typeof this.client.ensureSeeded === "function") return this.client.ensureSeeded();
    return false;
  }
  isEphemeral() {
    try { return !!(this.client && this.client.ephemeral); } catch { return false; }
  }
  ...
  async markNotesRead(dest) { return this.client.markNotesRead(dest); }

  // Gold
  async getGoldReqs() { return this.client.getGoldReqs(); }
  async createGoldReq(r) { return this.client.createGoldReq(r); }
  async updateGoldReq(id, status) { return this.client.updateGoldReq(id, status); }
  async updateAccountPassword(email, hash) { return this.client.updateAccountPassword(email, hash); }
  async getReactions(msgId) { return this.client.getReactions(msgId); }
  async toggleReaction(msgId, userKey, emoji) { return this.client.toggleReaction(msgId, userKey, emoji); }
  async getMessageById(id) { return this.client.getMessageById(id); }
  async deleteMessage(id) { return this.client.deleteMessage(id); }
  async markMessagesRead(threadUser, peer) { return this.client.markMessagesRead(threadUser, peer); }
  async updateFriendReq(id, status) { return this.client.updateFriendReq(id, status); }
  async getPrivAuth(user) { return this.client.getPrivAuth(user); }
  async setPrivAuth(user, patch) { return this.client.setPrivAuth(user, patch); }
  async getModActions() { return this.client.getModActions(); }
  async createModAction(a) { return this.client.createModAction(a); }
  async getAuditLog() { return this.client.getAuditLog(); }
  async createAuditLog(a) { return this.client.createAuditLog(a); }
